...
%conditions
% based on bg-quadrant


function main_visRes_bgqMedSplit(path)
%%
            ...
            for med = 1:2%1:low, 2:high bg val for each voxel
                for valType = 1:length(valIndx)%median-split by 1:rel, 2:irrel, 3:rel-irrel values
                    for rQuad = 1:length(recROI{roi})%collapsing for all sub-regions' index, note that potential index overlap is ignored
                        ...
                        act_med_valType_roi_samDiff_quad_scan{med}{valType}(roi, (sameDiff-1)*4+rQuad, scan) = mean(xVol(tempIndx));
                    end%for rQuad
                end%for valType 
            end%for med
        end%for roi
...
for med = 1:2%1:low, 2:high bg val for each voxel
    for valType = 1:length(valIndx)%median-split by 1:rel, 2:irrel, 3:rel-irrel values
        ...
        tempAct = mean(act_med_valType_roi_samDiff_quad_scan{med}{valType},3);%averaging across scans
        meanAct_med_valType_roi_SinSamDiff{valType}(:,(med-1)*3+2) = mean(tempAct(:,1:4),2);%same
        meanAct_med_valType_roi_SinSamDiff{valType}(:,(med-1)*3+3) = mean(tempAct(:,5:8),2);%same
    end%for valType
end%for med
%% save results
% [ST, I] = dbstack;
...
save([output_dir '/act_med_valType_roi_samDiff_quad_scan.mat'], 'act_med_valType_roi_samDiff_quad_scan');
