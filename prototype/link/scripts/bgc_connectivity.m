...
function bgc_connectivity(path)
%%
...
for scan = 1:var.BG.nScan%note that this scan number is 1~4
    xMat = BGMat{1}{scan};%center
    upCateInfo(scan) = unique(xMat(bg.upCate,:));
    tempOnset = xMat(bg.onset, :);
    volInfo{scan} = (tempOnset-time.BG.disDaq)/time.TR + 1;%into vol #
    
    volInfo_rest{scan} = setdiff(1:time.BG.runLeng(2), volInfo{scan});%rest vol#
end%for scan
...
for cate = 1:var.BG.nUpper%task category, 1:face, 2:scene
    ...
    for scan = 1:length(xCateScans)%order of each cagegory (e.g., first and second runs of face)
        ...
        xRes = [path.analysis.firstlevel '/bgc_0' num2str(xScan) '.feat/stats/res4d2standard.nii'];
        if ~exist(xRes)
            gunzip([xRes '.gz']);
        end%if ~exist
...
%             %stim blocks
%             recAct = [recResAct_vd{cate}{1}{recRoi}(ventDor,volInfo{xCateScans(1)})...
%                 recResAct_vd{cate}{2}{recRoi}(ventDor,volInfo{xCateScans(2)})];%collapsing first and second runs
            
            %stim blocks (z-scoring within each run)
            recAct = [zscore(recResAct_vd{cate}{1}{recRoi}(ventDor,volInfo{xCateScans(1)}))...
                zscore(recResAct_vd{cate}{2}{recRoi}(ventDor,volInfo{xCateScans(2)}))];%collapsing first and second runs
            
%              %rest blocks
%             recAct_rest = [recResAct_vd{cate}{1}{recRoi}(ventDor,volInfo_rest{xCateScans(1)})...
%                 recResAct_vd{cate}{2}{recRoi}(ventDor,volInfo_rest{xCateScans(2)})];%collapsing first and second runs
            
            %rest blocks(z-scoring within each run)
            recAct_rest = [zscore(recResAct_vd{cate}{1}{recRoi}(ventDor,volInfo_rest{xCateScans(1)}))...
                zscore(recResAct_vd{cate}{2}{recRoi}(ventDor,volInfo_rest{xCateScans(2)}))];%collapsing first and second runs
           
            
            for seedRoi = 1:length(seedsROIs_name)
%                 %stim blocks
%                 seedAct = [seedResAct{cate}{1}(seedRoi,volInfo{xCateScans(1)})...
%                     seedResAct{cate}{2}(seedRoi,volInfo{xCateScans(2)})];%collapsing first and second runs
                
                %stim blocks(z-scoring within each run)
                seedAct = [zscore(seedResAct{cate}{1}(seedRoi,volInfo{xCateScans(1)}))...
                    zscore(seedResAct{cate}{2}(seedRoi,volInfo{xCateScans(2)}))];%collapsing first and second runs
                
%                 %rest blocks
%                 seedAct_rest = [seedResAct{cate}{1}(seedRoi,volInfo_rest{xCateScans(1)})...
%                     seedResAct{cate}{2}(seedRoi,volInfo_rest{xCateScans(2)})];%collapsing first and second runs
                
                %rest blocks(z-scoring within each run)
                seedAct_rest = [zscore(seedResAct{cate}{1}(seedRoi,volInfo_rest{xCateScans(1)}))...
                    zscore(seedResAct{cate}{2}(seedRoi,volInfo_rest{xCateScans(2)}))];%collapsing first and second runs
                
                bgCorr{recRoi}{ventDor}(seedRoi, cate) = corr(recAct', seedAct');
                bgCorr_rest{recRoi}{ventDor}(seedRoi, cate) = corr(recAct_rest', seedAct_rest');
            end%for seedRoi
        end%for ventDor
    end%for recRoi
end%for cate
...
for cate = 1:var.BG.nUpper%1:face, 2:scene
    ...
    for recRoi = 1:length(recRois_name)-2%v1~v4, no ffa and ppa
        for quad = 1:length(recRois_name{roi})%for each quadrant!!!
            ...
            recPat = [zscore(recResPat{cate}{1}{recRoi}{quad}(:,volInfo{xCateScans(1)}),0,2)...
                zscore(recResPat{cate}{2}{recRoi}{quad}(:,volInfo{xCateScans(2)}),0,2)];%collapsing first and second runs, nVox x time
            
            seedAct_rel = [zscore(seedResAct{cate}{1}(cate,volInfo{xCateScans(1)}))...
                zscore(seedResAct{cate}{2}(cate,volInfo{xCateScans(2)}))];%e.g., ffa acts for face runs
            seedAct_irrel = [zscore(seedResAct{cate}{1}(2/cate,volInfo{xCateScans(1)}))...
                zscore(seedResAct{cate}{2}(2/cate,volInfo{xCateScans(2)}))];%e.g., ppa acts for face runs
...
save([output_dir '/bgCorr.mat'], 'bgCorr');
save([output_dir '/bgCorr_rest.mat'], 'bgCorr_rest');
